...
echo "INFO: Testing measurements in containers equals number of PCR_Extends for vTPM 1.2; starting ${num} containers"
...
for ((i = 0; i < "${num}"; i++)); do
  NSID="${i}" FAILFILE="${FAILFILE}" \
    run_busybox_container_vtpm 0 ./measure.sh &
done
